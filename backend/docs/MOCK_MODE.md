...
| `POST /api/campaigns/<id>/publish` | Creates a real **PAUSED** Search campaign in Google Ads, stores the real `google_campaign_id`, sets `status=PAUSED` | Sets `status=PAUSED` with a fake numeric `google_campaign_id` (no API call) |
| `POST /api/campaigns/<id>/pause` | Sets the Google Ads campaign to PAUSED | Sets `status=PAUSED` (no API call) |
| `POST /api/campaigns/<id>/resume` | Sets the Google Ads campaign to ENABLED (status `ENABLED`) | Sets `status=ENABLED` (no API call) |
